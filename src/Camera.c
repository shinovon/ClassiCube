...
static void Camera_OnRawMovement(float deltaX, float deltaY, int deviceIndex) {
	...
	Camera_states[i].deltaX += deltaX; 
	Camera_states[i].deltaY += deltaY;
}
...
void Camera_KeyLookUpdate(float delta) {
	...
	amount = (Global_Camera.Sensitivity / 25.0f) * (1000 * delta);
	i = Game.CurrentState;

	if (Bind_IsTriggered[BIND_LOOK_UP])    Camera_states[i].deltaY -= amount;
	if (Bind_IsTriggered[BIND_LOOK_DOWN])  Camera_states[i].deltaY += amount;
	if (Bind_IsTriggered[BIND_LOOK_LEFT])  Camera_states[i].deltaX -= amount;
	if (Bind_IsTriggered[BIND_LOOK_RIGHT]) Camera_states[i].deltaX += amount;
}
...
static void PerspectiveCamera_GetProjection(struct Matrix* proj) {
	float fovy = Global_Camera.Fov * MATH_DEG2RAD;
...
static void PerspectiveCamera_GetView(struct Matrix* mat) {
	Vec3 pos = Global_Camera.CurrentPos;
	Vec2 rot = Global_Camera.Active->GetOrientation();

	Matrix_LookRot(mat, pos, rot);
	if (Game_ViewBobbing) Matrix_MulBy(mat, &Global_Camera.TiltM);
}
...
static void PerspectiveCamera_GetPickedBlock(struct RayTracer* t) {
	...
	Vec3 dir    = Vec3_GetDirVector(e->Yaw * MATH_DEG2RAD, e->Pitch * MATH_DEG2RAD + Global_Camera.TiltPitch);
...
static Vec2 PerspectiveCamera_GetMouseDelta(float delta) {
	float sensitivity = CAMERA_SENSI_FACTOR * Global_Camera.Sensitivity;
	static float speedX, speedY, newSpeedX, newSpeedY, accelX, accelY;
	int i = Game.CurrentState;
	Vec2 v;

	if (Global_Camera.Smooth) {
		accelX = (Camera_states[i].deltaX - speedX) * 35 / Global_Camera.Mass;
		accelY = (Camera_states[i].deltaY - speedY) * 35 / Global_Camera.Mass;
		...
		speedX = Camera_states[i].deltaX;
		speedY = Camera_states[i].deltaY;
	}

	v.x = speedX * sensitivity; 
	v.y = speedY * sensitivity;
	if (Global_Camera.Invert) v.y = -v.y;
	return v;
}
...
static void PerspectiveCamera_UpdateMouseRotation(struct LocalPlayer* p, float delta) {
	...
	if (Input_IsAltPressed() && Global_Camera.Active->isThirdPerson) {
...
static void PerspectiveCamera_UpdateMouse(struct LocalPlayer* p, float delta) {
	...
	Camera_states[i].deltaX = 0; 
	Camera_states[i].deltaY = 0;
}
...
static void PerspectiveCamera_CalcViewBobbing(struct LocalPlayer* p, float t, float velTiltScale) {
	...
	if (!Game_ViewBobbing) {
		Global_Camera.TiltM     = Matrix_Identity;
		Global_Camera.TiltPitch = 0.0f;
		return;
	}
	...
	Matrix_RotateZ(&Global_Camera.TiltM, -xTilt                  * bobStrength);
	Matrix_RotateX(&tiltY,        Math_AbsF(yTilt) * 3.0f * bobStrength);
	Matrix_MulBy(&Global_Camera.TiltM, &tiltY);

	Global_Camera.BobbingHor = (bobbingHor * 0.3f) * bobStrength;
	Global_Camera.BobbingVer = (bobbingVer * 0.6f) * bobStrength;
	...
	Matrix_MulBy(&Global_Camera.TiltM, &velX);
	if (!Game_ClassicMode) Global_Camera.TiltPitch = fall;
}
...
static Vec3 FirstPersonCamera_GetPosition(float t) {
	...
	camPos.y += Global_Camera.BobbingVer;
	camPos.x += Global_Camera.BobbingHor * Math_CosF(yaw);
	camPos.z += Global_Camera.BobbingHor * Math_SinF(yaw);
	return camPos;
}
...
static Vec3 ThirdPersonCamera_GetPosition(float t) {
	...
	target.y += Global_Camera.BobbingVer;

	rot = Global_Camera.Active->GetOrientation();
...
static void OnRawMovement(void* obj, float deltaX, float deltaY) {
	Global_Camera.Active->OnRawMovement(deltaX, deltaY, 0);
}
...
static void OnAxisUpdate(void* obj, struct PadAxisUpdate* upd) {
	...
	Global_Camera.Active->OnRawMovement(upd->x, upd->y, upd->port);
}
...
void Camera_CycleActive(void) {
	...
	Global_Camera.Active = Global_Camera.Active->next;

	if (!p->Hacks.CanUseThirdPerson || !p->Hacks.Enabled) {
		Global_Camera.Active = &cam_FirstPerson;
	}
	cam_isForwardThird = Global_Camera.Active == &cam_ForwardThird;

	if (Global_Camera.Active == &cam_FirstPerson) cycle = 0;
	else if (Global_Camera.Active == &cam_ThirdPerson) cycle = 1;
...
void Camera_CheckFocus(void) {
	...
	if (focus) {
		Global_Camera.Active->AcquireFocus();
	} else {
		Global_Camera.Active->LoseFocus();
	}
}

void Camera_SetFov(int fov) {
	if (Global_Camera.Fov == fov) return;
	Global_Camera.Fov = fov;
	Camera_UpdateProjection();
}

void Camera_UpdateProjection(void) {
	if (!Global_Camera.Active) return;
	Global_Camera.Active->GetProjection(&Gfx.Projection);
...
static void ZoomScrollReleased(int key, struct InputDevice* device) {
	Camera_SetFov(Global_Camera.DefaultFov);
}
...
#ifdef CC_BUILD_WIN
	Global_Camera.Sensitivity = Options_GetInt(OPT_SENSITIVITY, 1, 200, 40);
#elif defined CC_BUILD_SYMBIAN_3 || defined CC_BUILD_SYMBIAN_S60V5 || defined CC_BUILD_WINCE
	Global_Camera.Sensitivity = Options_GetInt(OPT_SENSITIVITY, 1, 200, 70);
#else
	Global_Camera.Sensitivity = Options_GetInt(OPT_SENSITIVITY, 1, 200, 30);
#endif
	Global_Camera.Clipping    = Options_GetBool(OPT_CAMERA_CLIPPING, true);
	Global_Camera.Invert      = Options_GetBool(OPT_INVERT_MOUSE, false);
	Global_Camera.Mass        = Options_GetFloat(OPT_CAMERA_MASS, 1, 100, 20);
	Global_Camera.Smooth      = Options_GetBool(OPT_CAMERA_SMOOTH, false);

	Global_Camera.DefaultFov  = Options_GetInt(OPT_FIELD_OF_VIEW, 1, 179, 70);
	Global_Camera.Fov         = Global_Camera.DefaultFov;
	Global_Camera.ZoomFov     = Global_Camera.DefaultFov;
	Camera_UpdateProjection();
}
